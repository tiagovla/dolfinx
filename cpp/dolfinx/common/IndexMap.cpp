    ...
    const xtl::span<const std::int32_t>& indices, const IndexMap& map)
{
  ...
  const int num_owned = std::count_if(
      indices.begin(), indices.end(),
      [size_local](std::int32_t index) { return index < size_local; });
...
IndexMap::scatter_fwd_receive_offsets() const noexcept
{
  return _displs_recv_fwd;
}
//-----------------------------------------------------------------------------
const std::vector<std::int32_t>&
